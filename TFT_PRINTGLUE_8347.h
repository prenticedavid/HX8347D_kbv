...
class TFT_PRINTGLUE_8347 : public HX8347D_kbv
{
    public:
        TFT_PRINTGLUE_8347() {}
        void begin(void) {
        ...
        {
            HX8347D_kbv::reset();
            _ID = HX8347D_kbv::readID();
//            if (_ID == 0xD3D3) _ID = 0x9486;
            HX8347D_kbv::begin(_ID);
            HX8347D_kbv::setRotation(1);
            _first = true;
            _datum = TL_DATUM;
        }
        void setWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
        {
            HX8347D_kbv::setAddrWindow(x0, y0, x1, y1);
            _first = true;
        }

        void pushColors(uint8_t *data, uint16_t len)
        {
            HX8347D_kbv::pushColors((uint8_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint8_t len)
        {
            HX8347D_kbv::pushColors((uint16_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint16_t len, int first)
        {
            HX8347D_kbv::pushColors((uint16_t*)data, len, first);
        }

        void pushColor(uint16_t color)
        {
            uint16_t c = color;
            HX8347D_kbv::pushColors(&c, 1, _first);
            _first = false;
        }
        ...
        void setCursor(int16_t x, int16_t y, uint8_t idx = 255)
        {
            ...
            setTextFont(idx);
            HX8347D_kbv::setCursor(x, y + _ydatum);
        }
        ...
        void setFreeFont(const GFXfont *pfont)
        {
            ...
            _ascent = 7;
            _descent = 1;
            HX8347D_kbv::setFont(_pfont = pfont);
            if (pfont != NULL) {
                int16_t x = 0, y = 0, x1, y1 = 0, x2, y2 = 0;
                HX8347D_kbv::setTextSize(1);
        ...
        int16_t drawString(const char *string, int16_t poX, int16_t poY, uint8_t idx)
        {
            ...
            HX8347D_kbv::setCursor(poX, poY + _ydatum);
            print(string);
            return w;
        }
